...
| Rama | Capa visual |
|---|---|
| `main` · `tailwind` | Design system propio: Tailwind CSS 4, tokens `oklch`, clases `btn-*`/`th-ds`, iconos lucide — **es lo que describe este README** |
| `angular-material` | Angular Material 3: tokens `--mat-sys-*` emitidos por `mat.theme()`, Material Symbols |

`tailwind` es una copia estable de `main` para que esta línea siga viva aunque
